...
import { Camera, BookOpen, ScanBarcode, Mic, Zap, Search } from 'lucide-react';
...
const AppContent = () => {
  ...
  const [pendingImageLog, setPendingImageLog] = useState(null);
  ...
  const handleFoodSearchAdd = (item) => {
    addLog({ ...item, timestamp: currentDate });
  };
  ...
  const handleSend = async (data, type, history = []) => {
    ...
    if (type === 'text') {
      ...
      const transcription = await transcribeAudio(settings.apiKey, settings.selectedModel, data.audio, data.mimeType);
    ...
    if (result && !result.error) {
      ...
      // For image results: show review modal when there are multiple items or any low/medium confidence
      if (type === 'image' && items.length > 0) {
        const needsReview = items.length > 1 || items.some(i => i.confidence !== 'high');
        if (needsReview) {
          setPendingImageLog({ items, mealType: result.meal_type, mealName: result.meal_name });
          return { ...baseResponse, message: result.status_message || "Please review the detected items before logging." };
        }
      }
  ...
  return (
    <Layout onOpenSettings={() => setShowSettings(!showSettings)}>
      ...
      {/* Persistent Chat Input Trigger / FAB */}
        ...
        <div className="max-w-md mx-auto bg-neutral-800/90 backdrop-blur-md border border-neutral-700 p-2 rounded-3xl flex items-center gap-2 shadow-2xl cursor-pointer" onClick={() => openChatWithMode(null)}>
          <div className="flex gap-0.5">
            <button
              ...
              className="p-2.5 text-neutral-400 hover:text-white hover:bg-neutral-700/50 rounded-xl transition-colors"
            >
              <Camera size={19} />
            </button>
            <button
            ...
            <button
              ...
              className="p-2.5 text-amber-500/70 hover:text-amber-400 hover:bg-neutral-700/50 rounded-xl transition-colors"
              title="Meal Templates"
            >
              <Zap size={19} />
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); setShowFoodSearch(true); }}
              className="p-2.5 text-neutral-400 hover:text-white hover:bg-neutral-700/50 rounded-xl transition-colors"
        ...
        <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowSettings(false)}>
          <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-3xl w-full max-w-xs" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4 space-y-3">
              ...
              <div>
                <label className="text-xs text-neutral-500 font-bold block mb-1">MODEL</label>
                <select
      ...
      {showTemplates && <MealTemplates onClose={() => setShowTemplates(false)} />}
        ...
        <ConfirmLogModal
          ...
          onConfirm={(editedItems) => {
            if (editedItems.length > 1) {
              const totalCalories = editedItems.reduce((s, i) => s + (i.calories || 0), 0);
              const totalProtein = editedItems.reduce((s, i) => s + (i.protein || 0), 0);
              const totalCarbs = editedItems.reduce((s, i) => s + (i.carbs || 0), 0);
              const totalFat = editedItems.reduce((s, i) => s + (i.fat || 0), 0);
              const mealName = pendingImageLog.mealName || editedItems.map(i => i.food_name).join(' & ').substring(0, 30) + '...';
              addLog({
                food_name: mealName,
                calories: totalCalories,
                protein: totalProtein,
                carbs: totalCarbs,
                fat: totalFat,
                quantity: `${editedItems.length} items`,
                source: 'image',
                timestamp: currentDate,
                items: editedItems.map(i => ({ ...i, id: uuidv4() })),
                meal_type: pendingImageLog.mealType
              });
            } else {
            ...
            setPendingImageLog(null);
          }}
          onCancel={() => setPendingImageLog(null)}
        />
      )}
